...
- (id)initClockWithLabels:(BOOL)showsLabels showsSeconds:(BOOL)showsSeconds {
    ...
    if (self) {
        ...
        _timer = [NSTimer scheduledTimerWithTimeInterval:_showsSeconds ? 1 : 60 target:self selector:@selector(flipClock) userInfo:nil repeats:YES];
        if (!_showsSeconds) {
...
- (void)initializeAppearanceWithDays:(NSInteger)days hours:(NSInteger)hours minutes:(NSInteger)minutes seconds:(NSInteger)seconds {
    ...
    _hour = [[KDFlipClockSegment alloc] initWithDigits:hours];
    ...
    _min = [[KDFlipClockSegment alloc] initWithDigits:minutes];
    ...
    if (_showsSeconds) {
        _sec = [[KDFlipClockSegment alloc] initWithDigits:seconds];
...
- (void)flipClock {
    ...
    [_hour flipWithDigits:comps.hour];
    [_min flipWithDigits:comps.minute];
    if (_showsSeconds) [_sec flipWithDigits:comps.second];
}
